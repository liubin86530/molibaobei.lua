...
function commandsNormal.pi(charIndex, args)
  local windowStr="@c请输入宠物图档编号，将会为您第一栏的宠物更换形象"
  NLG.ShowWindowTalked(charIndex, npc, CONST.窗口_输入框, CONST.BUTTON_确定关闭, 1,windowStr);
...
function commandsNormal.speedup(charIndex, args)

  local command = args[1]
  if(command =='on') then

    local charPtr = Char.GetCharPointer(charIndex)
    ffi.setMemoryInt32(charPtr + 0x5e8 + 0x188 + 0x18, speedRate);   --walkSpeed
...
function commandsNormal.hc(charIndex)
  --Char.DischargeParty(charIndex);
  Char.Warp(charIndex, 0, 1000, 233, 80);
end
...
function ngModule:onNpcTalked(npc, charIndex, seqno, select, data)
  ...
  if petIndex<0 then
    NLG.Say(charIndex,-1,"[错误] 未找到第一栏宠物",CONST.颜色_红色,0)
    return 
  end

  Char.SetData(petIndex, CONST.对象_原始图档, data)
  Char.SetData(petIndex, CONST.对象_原形, data)
  Pet.UpPet(charIndex,petIndex);
  NLG.Say(charIndex,-1,"第一栏的宠物有了新的形象",CONST.颜色_黄色,0)
end
--- 加载模块钩子
function ngModule:onLoad()
  ...
  npc = self:NPC_createNormal('宠物图档', 110308, { x = 20, y = 15, map = 777, mapType = 0, direction = 0 });
  
  self:NPC_regWindowTalkedEvent(npc, Func.bind(self.onNpcTalked, self));
end
